 ...
 *
 */
public class BaselineAggregateAndRecommendHDFSReducer extends
        Reducer<VarLongWritable, PrefAndSimilarityColumnWritable, IntPairWritable, FloatWritable> {
    ...
    private void writeRecommendedItems(final VarLongWritable userID,
        ...
        if (!topItems.isEmpty()) {
            for (final RecommendedItem item : topItems) {
                context.write(new IntPairWritable((int) userID.get(), (int) item.getItemID()),
                        new FloatWritable(item.getValue()));
            }
        }
    }
