...
public class ComputeHJob extends MatrixComputationJob {
     ...
     */
    public ComputeHJob(final Path H, final Path W, final Path H2, final Path W2) {
        super(H, W, H2, W2);
    }
    ...
    public int run(final String[] args) throws Exception {
        ...
        directory = conf.get(RMRecommenderDriver.directory) + File.separator + "hcomputation";
     ...
     */
    protected void runJob1(final Path wPath, final Path xPath)
            throws IOException, ClassNotFoundException, InterruptedException {

        final Configuration conf = getConf();

        final Job job = new Job(HadoopUtils.sanitizeConf(conf),
                prefix + "H1" + cluster + "-it" + iteration);
     ...
     * @throws InterruptedException
     */
    protected void runJob2(final Path wPath, final Path cPath)
            throws IOException, ClassNotFoundException, InterruptedException {

        final Job job = new Job(HadoopUtils.sanitizeConf(getConf()),
                prefix + "H2" + cluster + "-it" + iteration);
        job.setJarByClass(this.getClass());
     ...
     * @throws InterruptedException
     */
    protected void runJob3(final Path hPath, final Path yPath, final Path cPath)
            throws IOException, ClassNotFoundException, InterruptedException {

        final Job job = new Job(HadoopUtils.sanitizeConf(getConf()),
                prefix + "H3" + cluster + "-it" + iteration);
        job.setJarByClass(this.getClass());
