...
public class BaselineRecommenderJob extends AbstractJob {
     ...
     *            JobContext
     */
    private static void setIOSort(final JobContext job) {
        ...
        if (javaOpts != null) {
            final Matcher m = Pattern.compile("-Xmx([0-9]+)([mMgG])").matcher(javaOpts);
            if (m.find()) {
                assumedHeapSize = Integer.parseInt(m.group(1));
                final String megabyteOrGigabyte = m.group(2);
    ...
    protected void loadDefaultSetup() {
        ...
        addOption("maxPrefsPerUserInItemSimilarity", "mppuiis",
        ...
        addOption("threshold", "tr", "discard item pairs with a similarity value below this",
                false);
        ...
        addOption("tableOut", "tout", "Cassandra output column family", "recommendations");
    ...
    public int run(final String[] args) throws Exception {
        ...
        final int maxPrefsPerUser = Integer.parseInt(getOption("maxPrefsPerUser"));
        final int minPrefsPerUser = Integer.parseInt(getOption("minPrefsPerUser"));
        final int maxPrefsPerUserInItemSimilarity = Integer
                .parseInt(getOption("maxPrefsPerUserInItemSimilarity"));
        final int maxSimilaritiesPerItem = Integer.parseInt(getOption("maxSimilaritiesPerItem"));
        final String similarityClassname = getOption("similarityClassname");
        final double threshold = hasOption("threshold") ? Double.parseDouble(getOption("threshold"))
                : RowSimilarityJob.NO_THRESHOLD;

        final Path prepPath = getTempPath("preparePreferenceMatrix");
        final Path similarityMatrixPath = getTempPath("similarityMatrix");
        final Path partialMultiplyPath = getTempPath("partialMultiply");

        final AtomicInteger currentPhase = new AtomicInteger();

        int numberOfUsers = -1;

        if (shouldRunNextPhase(parsedArgs, currentPhase)) {
            ToolRunner.run(getConf(), new BaselinePreparePreferenceMatrixJob(),
                    new String[] { "--output", prepPath.toString(), "--maxPrefsPerUser",
                            String.valueOf(maxPrefsPerUserInItemSimilarity), "--minPrefsPerUser",
                            String.valueOf(minPrefsPerUser), "--booleanData",
                            String.valueOf(booleanData), "--tempDir", getTempPath().toString(),
                            "--keyspace", keyspace, "--table", tableIn });

            numberOfUsers = HadoopUtil
                    .readInt(new Path(prepPath, PreparePreferenceMatrixJob.NUM_USERS), getConf());
        }
        ...
        if (shouldRunNextPhase(parsedArgs, currentPhase)) {
            ...
            if (numberOfUsers == -1) {
                numberOfUsers = (int) HadoopUtil.countRecords(
                        new Path(prepPath, PreparePreferenceMatrixJob.USER_VECTORS), PathType.LIST,
                        null, getConf());
            }
            ...
            ToolRunner
                    .run(getConf(), new RowSimilarityJob(),
                            new String[] { "--input",
                                    new Path(prepPath, PreparePreferenceMatrixJob.RATING_MATRIX)
                                            .toString(),
                                    ...
                                    "--threshold", String.valueOf(threshold), "--tempDir",
            ...
            if (hasOption("outputPathForSimilarityMatrix")) {
                final Path outputPathForSimilarityMatrix = new Path(
                        getOption("outputPathForSimilarityMatrix"));

                final Job outputSimilarityMatrix = prepareJob(similarityMatrixPath,
                        outputPathForSimilarityMatrix, SequenceFileInputFormat.class,
                ...
                final Configuration mostSimilarItemsConf = outputSimilarityMatrix
                        .getConfiguration();
         ...
         */
        if (shouldRunNextPhase(parsedArgs, currentPhase)) {
            final Job partialMultiply = new Job(getConf(), "partialMultiply");
            final Configuration partialMultiplyConf = partialMultiply.getConfiguration();

            MultipleInputs.addInputPath(partialMultiply, similarityMatrixPath,
                    SequenceFileInputFormat.class, SimilarityMatrixRowWrapperMapper.class);
            MultipleInputs.addInputPath(partialMultiply,
                    new Path(prepPath, PreparePreferenceMatrixJob.USER_VECTORS),
            ...
            final boolean succeeded = partialMultiply.waitForCompletion(true);
            if (!succeeded) {
                return -1;
            }
        }
     ...
     */
    protected int selectTopRecommendations(final Path partialMultiplyPath, final Path prepPath)
            throws IOException, ClassNotFoundException, InterruptedException {

        final String aggregateAndRecommendInput = partialMultiplyPath.toString();

        final Job aggregateAndRecommend = new Job(new Configuration(getConf()),
                "aggregateAndRecommend");

        aggregateAndRecommend.setMapperClass(PartialMultiplyMapper.class);
        aggregateAndRecommend.setReducerClass(BaselineAggregateAndRecommendReducer.class);
        ...
        aggregateAndRecommend.setMapOutputValueClass(PrefAndSimilarityColumnWritable.class);
        aggregateAndRecommend.setOutputKeyClass(Map.class);
        aggregateAndRecommend.setOutputValueClass(List.class);

        aggregateAndRecommend.setInputFormatClass(SequenceFileInputFormat.class);
        ...
        final Configuration conf = aggregateAndRecommend.getConfiguration();
        ...
        final String query = "UPDATE " + keyspace + "." + tableOut + " USING TTL " + ttl
                + " SET score = ? ";
        ...
        conf.set(ITEMID_INDEX_PATH,
                new Path(prepPath, PreparePreferenceMatrixJob.ITEMID_INDEX).toString());
        conf.setInt(NUM_RECOMMENDATIONS, numRecommendations);
        conf.setBoolean(BOOLEAN_DATA, booleanData);
        final boolean succeeded = aggregateAndRecommend.waitForCompletion(true);
        if (!succeeded) {
            return -1;
        }
