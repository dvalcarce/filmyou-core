 ...
 *
 */
public class BaselinePreparePreferenceMatrixJob extends PreparePreferenceMatrixJob {
    ...
    public static void main(final String[] args) throws Exception {
        if (ToolRunner.run(new BaselinePreparePreferenceMatrixJob(), args) < 0) {
            throw new RuntimeException("BaselinePreparePreferenceMatrixJob failed!");
        }
    }
     ...
     */
    protected int createUserVectors()
            throws IOException, ClassNotFoundException, InterruptedException {

        final Job toUserVectors = new Job(new Configuration(getConf()), "toUserVectors");
        ...
        toUserVectors.setMapOutputValueClass(
        ...
        final Configuration conf = toUserVectors.getConfiguration();
        ...
        final int numberOfUsers = (int) toUserVectors.getCounters()
        ...
        final Job toItemVectors = prepareJob(getOutputPath(USER_VECTORS),
                getOutputPath(RATING_MATRIX), ToItemVectorsMapper.class, IntWritable.class,
                VectorWritable.class, BaselineToItemVectorsReducer.class, IntWritable.class,
        ...
        if (hasOption("maxPrefsPerUser")) {
            final int samplingSize = Integer.parseInt(getOption("maxPrefsPerUser"));
     ...
     * @return int succeed
     */
    protected int getMinimumMR() throws ClassNotFoundException, IOException, InterruptedException {

        final Job itemIDIndex = new Job(new Configuration(getConf()), "itemIDIndex");
        ...
        final Configuration conf = itemIDIndex.getConfiguration();
        ...
        final boolean succeeded = itemIDIndex.waitForCompletion(true);
        if (!succeeded) {
            return -1;
        }
        return 0;
    ...
    public int run(final String[] args) throws Exception {
        ...
        final Map<String, List<String>> parsedArgs = parseArguments(args, true, false);
        if (parsedArgs == null) {
            return -1;
        }
